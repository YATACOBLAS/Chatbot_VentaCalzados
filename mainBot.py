...
if path.exists("modelo.tflearn.meta"):
    ...
    modelo.fit(entrenamiento,salida,n_epoch=1100, batch_size=50,show_metric=True)
...
@app.route('/api/pedir_chatbot',methods = ['GET', 'POST'])
def recipes():
    # recipes
    recipes = [
...
def about():
		...
		mensaje_respuesta = mainBot(mensaje_usuario)
		flash(mensaje_respuesta,category="info")
